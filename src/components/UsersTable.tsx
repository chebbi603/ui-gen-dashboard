...
export default function UsersTable({ onSelectUser, selectedUserId }: UsersTableProps) {
      ...
      <CardHeader className="flex items-center justify-between px-4">
        ...
        <div className="flex items-center gap-3">
          <div className="relative w-64">
            <IconSearch className="absolute left-2 top-1/2 -translate-y-1/2 size-4 text-muted-foreground" aria-hidden />
            <Input
        ...
        {error && (
          <div className="flex items-center justify-between rounded-md border border-destructive px-3 py-2 text-destructive">
            <div className="flex items-center gap-2 text-sm">
              ...
              <span>Failed to load users. Please check backend connectivity.</span>
            </div>
            <Button size="sm" variant="outline" onClick={loadUsers} aria-label="Retry loading users">
              Retry
            </Button>
          </div>
        )}
            ...
            {filtered.length === 0 && !loading && !error && (
              <TableRow>
                <TableCell colSpan={4} className="text-center text-sm text-muted-foreground">
                  {total === 0 ? "No users available" : "No users found. Try a different search term."}
                </TableCell>
              </TableRow>
            )}
            {filtered.map((u) => {
              ...
              return (
                ...
                >
                  <TableCell className={selected ? "font-semibold" : "font-medium"}>{u.name}</TableCell>
                  <TableCell className="text-sm">{u.email ?? "—"}</TableCell>
                  <TableCell className="text-sm">{version}</TableCell>
                  <TableCell className="text-right">
                    <Button size="sm" onClick={() => onSelectUser(u.id)} aria-label={`View details for ${u.name}`}>
                      View Details
                    </Button>
                  </TableCell>
                </TableRow>
